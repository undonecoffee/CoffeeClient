...
import HandleGui from "../DocGuiLib/Gui"
...
import { CustomGui } from "../DocGuiLib/CustomGui"
import HandleRegisters from "../DocGuiLib/Registers"
...
const mergeObjects = (obj1, obj2, final = {}) => {
    ...
    for (let entry of Object.entries(obj2)) {
        ...
        if (k in final) {
            ...
            if (typeof v == "object" && !Array.isArray(v)) final[k] = mergeObjects(obj1[k], v)
            continue
        }
...
export default class Settings {
    ...
    constructor(moduleName, defaultConfig) {
        ...
        this.colorSchemePath = "./scheme.json"
    ...
    setConfigValue(category, configName, value) {
        ...
        if (!configObj) {
            ...
            if (configObj?.type === ConfigTypes.MULTICHECKBOX) configObj = configObj.options.find(it => it.configName === configName)
        }
     ...
     */
    _checkScheme(path) {
        const mainDefaultScheme = JSON.parse(FileLib.read("CoffeeClient", "guttedAmaterasu/DocGuiLib/DefaultColors.json"))
        let defaultScheme = JSON.parse(FileLib.read("CoffeeClient", "guttedAmaterasu/amaterasu/scheme.json"))
